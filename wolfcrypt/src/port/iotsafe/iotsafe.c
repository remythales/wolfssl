...
//hkdf extract
static int iotsafe_hkdf_extract(byte* prk, const byte* salt, int saltLen,
...
static int wolfIoT_hkdf_extract(WOLFSSL* ssl,
       ...
       void* ctx)
{
    ...
    printf("\nhash: %d\n", mac);
     ...
     if (iotsafe->enabled) {
           ret = iotsafe_hkdf_extract(prk, salt, saltLen, ikm, ikmLen, mac);
        return ret;
    }
    else
